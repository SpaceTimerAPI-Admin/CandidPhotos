  ...
  <script>
    ...
    document.getElementById('unlockBtn').addEventListener('click', async () => {
      ...
      if (j.url) {
        ...
        img.classList.remove('blurred'); // unblur after "I followed"
      } else {
...
export default async (request) => {
  ...
  // Optional: set title if record exists
  if (id && id !== "share") {
    const { data: row } = await supabase.from("photos").select("*").eq("id", id).maybeSingle();
    return new Response(page(row ? "Photo Download" : "Link not found"), { headers: { "content-type": "text/html; charset=utf-8" } });
  }
  return new Response(page("Missing ID"), { headers: { "content-type": "text/html; charset=utf-8" } });
};
