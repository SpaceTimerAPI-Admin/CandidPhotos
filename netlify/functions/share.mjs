...
<body>
  <div class="card">
    ...
    <div class="content">
      ...
      <div class="preview">
        <img id="thumb" alt="Photo preview"/>
      </div>
      <div class="gate">
  ...
  <script>
    ...
    // Primary: query string ?id=...
    let id = u.searchParams.get('id');
    // Fallback: last segment in path (so /share/:id works even if redirect fails)
    if (!id) {
...
export default async (request) => {
  ...
  // Prefer query param (how redirects deliver it)
  let id = url.searchParams.get("id");

  // Fallback to path segment if someone hits function URL directly routed by framework
  if (!id) {
    const segs = url.pathname.split("/").filter(Boolean);
    // Try last segment if it looks like a UUID
    const last = segs[segs.length - 1];
    if (last && last.length >= 20) id = last;
  }

  // If still no id, return page with helpful message
  if (!id) return new Response(page("Missing ID"), { headers: { "content-type": "text/html; charset=utf-8" } });

  // Optional: set proper title if row exists
  const { data: row } = await supabase.from("photos").select("*").eq("id", id).maybeSingle();
  const html = page(row ? "Photo Download" : "Link not found");
